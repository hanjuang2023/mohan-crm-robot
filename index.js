...
async function startWA(phone = null) {
    ...
    sock.ev.on("connection.update", (up) => {
        if(up.connection === "open") status = "Aktif";
        if(up.connection === "close") status = "Mati";
    });
...
app.get("/status", (req, res) => res.json({ status: status }));
...
app.listen(3000, () => { console.log("WA CLONE ENGINE ONLINE"); startWA(); });
